...
echo "CHECK connection and grants on SOURCE ${SOURCE_MYSQL_HOST}"
${DOCKER_CMD} mysql --host=${SOURCE_MYSQL_HOST} --user=${DB_USER} --password=${DB_PASS} -e 'show tables;' ${DB}

# CHECK TARGET_MYSQL_HOST if database exists
echo "CHECK connection and grants toon TARGET ${TARGET_MYSQL_HOST}"
${DOCKER_CMD} mysql --host=${TARGET_MYSQL_HOST} --user=${DB_USER} --password=${DB_PASS} -e 'show tables;' ${DB}
...
echo	${DOCKER_CMD} mysqldump --host=${SOURCE_MYSQL_HOST} --user=${DB_USER} --password=${DB_PASS} ${DUMP_OPTS} ${DB} ${TABLE} | ${DOCKER_CMD} mysql --host=${TARGET_MYSQL_HOST} --user=${DB_USER} --password=${DB_PASS} --init-command="SET SESSION FOREIGN_KEY_CHECKS=0; SET SESSION UNIQUE_CHECKS=0;" $DB
